...
"""
Module for calculation of the performance measures.
"""
...
if __name__ == "__main__":
    training_path = ["Data/audio/shakespeare.wav",
                     "Data/audio/WhiteNoise.wav"]
    rate, src_shakes_orig = wavfile.read(training_path[0])
    rate, src_noise_orig = wavfile.read(training_path[1])

    output_path = ["Data/audio_output/shakes_whitenoise/voice_nmf_(50,10)_a=100.wav",
                   "Data/audio_output/shakes_whitenoise/noise_onmf_(50,10)_a=100.wav"]

    rate, src_shakes = wavfile.read(output_path[0])
    rate, src_noise = wavfile.read(output_path[1])

    src_shakes_ori = src_shakes_orig[0: len(src_shakes)]
    src_noise_ori = src_noise_orig[0: len(src_noise)]

    audio_measures = []

    inst_num = 2
    siglen = len(src_shakes)
    synth_signals = np.zeros((inst_num, siglen))
    orig_signals = np.zeros((inst_num, siglen))

    synth_signals[0] = src_shakes
    synth_signals[1] = src_noise
    orig_signals[0] = src_shakes_ori
    orig_signals[1] = src_noise_ori

    perm, perf = select_perm(*measures(
